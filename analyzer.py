...
def sync_google_sheet_if_configured(config_path="google_sheet_config.json", stocks_csv_path="stocks.csv"):
    ...
    try:
        ...
        print(f"Fetching latest stock list from Google Sheet: {sheet_url}...")
        ...
        if resp.status_code == 200:
            ...
            for idx, r in enumerate(rows):
                ...
                sym = clean_symbol(r[0])
                if not sym or sym.startswith("SYMBOL"): continue
                name = r[1].strip() if len(r) > 1 else sym.split('.')[0]
                ...
                if sym not in seen:
                    ...
                    clean_stocks.append({
                        ...
                        "cap_type": "Equity",
                        "tracking_notes": notes or "Spark Google Sheet"
                    })
                    
            with open(stocks_csv_path, 'w', encoding='utf-8', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=["symbol", "name", "sector", "cap_type", "tracking_notes"])
                writer.writeheader()
                for s in clean_stocks:
                    writer.writerow(s)
            print(f"Successfully updated {stocks_csv_path} with {len(clean_stocks)} stocks from Google Sheet!")
    except Exception as e:
        print(f"Google Sheet sync warning: {e}")
...
def fetch_events_and_news(ticker, symbol, current_price, rev_growth_yoy, earnings_growth_yoy, dividend_yield):
    ...
    # 1. Dividend Ex-Dates & Announcements
    try:
        ...
        if ex_date_timestamp:
            ...
            if abs((ex_date - today).days) <= 7:
                ...
                events_list.append({
                    ...
                    "impact_reason": f"High dividend yield ({dividend_yield:.2f}%) attracts long-term income investors. Minor short-term price adjustment equal to dividend amount."
                })
    except Exception:
        pass

    # 2. Quarterly Earnings & Financial Results Event
    if rev_growth_yoy > 15 or earnings_growth_yoy > 20:
        ...
        events_list.append({
            ...
            "summary": f"Recent financial results show margin pressure with profit declining by {abs(earnings_growth_yoy):.1f}%.",
            "impact": "Bearish Caution ⚠️",
            "impact_reason": "Margin pressure may trigger short-term profit booking or institutional trimming."
        })

    # 3. Corporate Actions & News Stub
    try:
        ...
        for n in news_items[:2]:
            ...
            if pub_time:
                ...
                events_list.append({
                    ...
                    "type": "Corporate News",
                    "title": title,
                    "summary": f"Recent media coverage / announcement for {symbol.replace('.NS','')}.",
                    "impact": "Neutral / Watch ⚖️",
                    "impact_reason": "Monitored for corporate development and market sentiment impact."
                })
    except Exception:
        pass

    # Default Event if none logged
    if not events_list:
        events_list.append({
            "date": today.strftime("%Y-%m-%d"),
            "date_tag": "Today",
            ...
            "summary": f"Stock trading normally at ₹{current_price:.2f}. No pending corporate disclosures or splits.",
            "impact": "Neutral ⚖️",
...
def fetch_stock_data(symbol, metadata):
    ...
    try:
        ...
        print(f"Warning info failed for {symbol}: {e}")
    ...
    try:
        ...
        if q_financials is not None and not q_financials.empty:
            ...
            for idx in ['Total Revenue', 'Operating Revenue', 'Revenue']:
                if idx in q_financials.index:
                    rev_row = q_financials.loc[idx]
                    break
            if rev_row is not None and len(rev_row) >= 2:
                q_latest = clean_val(rev_row.iloc[0])
                q_prev = clean_val(rev_row.iloc[1])
                if q_prev > 0:
                    q_sales_growth = round(safe_pct_change(q_latest, q_prev), 1)

            pat_row = None
            for idx in ['Net Income', 'Net Income Common Stockholders']:
                if idx in q_financials.index:
                    pat_row = q_financials.loc[idx]
                    break
            if pat_row is not None and len(pat_row) >= 2:
                pat_latest = clean_val(pat_row.iloc[0])
                pat_prev = clean_val(pat_row.iloc[1])
                if pat_prev != 0:
                    q_pat_growth = round(safe_pct_change(pat_latest, pat_prev), 1)
    except Exception as e:
    ...
    return {
        ...
        "name": metadata.get('name', symbol.split('.')[0]),
        "sector": metadata.get('sector', 'General'),
        "cap_type": metadata.get('cap_type', 'Large Cap'),
        ...
        for row in reader:
            ...
            if s_val.strip():
                stock_meta = {
                    ...
                    "tracking_notes": row.get('tracking_notes') or row.get('Notes') or ''
                }
                stocks.append(stock_meta)
            
    analyzed = []
    for stock_meta in stocks:
        data = fetch_stock_data(stock_meta['symbol'], stock_meta)
        if data:
...
def run_analysis():
    ...
    sync_google_sheet_if_configured()
    
    analyze_file("stocks.csv", "analysis_data.json", "analysis_data.js", "stockData")
    analyze_file("nifty250.csv", "nifty250_data.json", "nifty250_data.js", "nifty250Data")
